...
describe('evaluation mutation suite', () => {
  it('kills every seeded evaluator defect with its expected detector', () => {
    ...
    expect(report.results.find((item) => item.id === 'wrong-adjustment')?.detectedBy).toContain('data');
    expect(report.results.find((item) => item.id === 'fabricated-review-citation')?.detectedBy).toContain('semantic');
    expect(report.results.find((item) => item.id === 'tampered-evaluator-score')?.detectedBy).toContain('report');
  });
});
