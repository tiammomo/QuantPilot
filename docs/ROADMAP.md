...
可复现的局部性能对照：`node -r tsconfig-paths/register --import tsx scripts/checks/benchmark-data-prefetch.ts`。8 个标的、每次合成接口延迟 50ms，三轮中位数由串行/重复行情的 1279ms 降为 463ms，请求 25 → 17，峰值并发 1 → 2，研究数据一致。该夹具不含真实供应商、LLM、验证和预览耗时；后续仍需真实研究样本、分阶段延迟和跨项目负载验证。PIT 参数完整传递与规划/预取持久任务化继续保持未完成状态。
...
- generation 与评测已由 PostgreSQL 队列和独立 Worker 执行；生成前的规划/预取、策略扫描和补数仍需持久化任务编排，暂停、恢复与事件语义仍待统一。
