...
2026-09-19 架构治理批次：补齐全部非测试源码的模块归属，新增持久化、身份配额、执行隔离和共享前端边界；共享上下文/用量合同从运行时和聊天模块迁出，AST 门禁强制跨模块公开接口。预取的时间范围与选股范围以已确认计划为准，原问题不再覆盖决定；选股细节结构化和历史时点完整传递仍待完善。评测移除数据库/JSON 双写和 Web 内存调度，新增 PostgreSQL 原子领取、租约、受保护的终态写入、定时入队事务及独立 Worker。报告绑定任务/租约/模式/模型，取消不再使用数据库历史 PID。失败任务需人工重新发起，未宣称全量实验重放或自动重试已完成。

2026-09-19 持久研究准备批次：Worker 模式在 HTTP 202 前保存原始指令、身份范围和附件引用；模型规划、Memory/Knowledge 和取数全部交由 Worker。准备完成以 dispatch fence 事务替换最终执行信封，重启后读取保存的输入；取消后的行情响应不落盘，在途请求结束后才释放工作区。准备阶段崩溃明确失败且不自动重计费，生成阶段保留有界接管。排队/早期失败可以直接从数据库恢复到页面。本批仅新增任务阶段约束迁移，无业务数据回填。
...
可复现的局部性能对照：`node -r tsconfig-paths/register --import tsx scripts/checks/benchmark-data-prefetch.ts`。8 个标的、每次合成接口延迟 50ms，三轮中位数由串行/重复行情的 1279ms 降为 463ms，请求 25 → 17，峰值并发 1 → 2，研究数据一致。该夹具不含真实供应商、LLM、验证和预览耗时；后续仍需真实研究样本、分阶段延迟和跨项目负载验证。PIT 参数完整传递继续保持未完成状态。
...
- generation 与评测已由 PostgreSQL 队列和独立 Worker 执行；Worker 模式已覆盖生成前的规划/上下文准备/预取，保存准备检查点后恢复执行；准备中断需新请求重试。策略扫描、补数和跨阶段细粒度恢复仍需完善。
