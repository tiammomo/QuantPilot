...
本教程中的 `.pi/**` 指仓库根目录受 registry/lock、版本与 SHA-256 完整性校验的 Skill 权威源，也是内置基线的 source-first 编译输入，目前没有密码学签名。项目初始化会把校验后的参考镜像配置到 `<workspace>/.pi/skills/`；Agent 执行阶段按平台固定版本记录只读编译上下文，不从 workspace 镜像发现能力。
...
以下步骤用于维护仓库内置基线。已启用在线版本管理的环境，还需要在 Studio 中发布对应草稿，并单独升级需要采用新版本的项目。
...
| 收起左侧列表 | 让编辑区域有更大空间 |
| 编辑源码与运行规则 | 保存隔离草稿；并发修改冲突时保留未提交文本 |
| 查看版本 | 理解最近改过什么 |
| 发布 | 确认 Diff，校验源码、脚本、注册信息和运行规则，再激活完整快照 |
| 回退 | 恢复完整发布版本；仅有旧源码包的历史版本不提供完整回退 |
| 项目安装 | 在 Market 中选择项目、Agent 和版本，支持卸载及回退上次安装 |

保存或上传只影响草稿；发布校验失败时当前版本保持不变。已固定版本的项目需要单独升级，详细流程见 [Skills 治理规范](../skills-governance.md)。
